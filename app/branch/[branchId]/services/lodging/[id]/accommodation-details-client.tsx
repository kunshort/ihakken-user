...
export default function AccommodationDetailsClient({
  ...
  const [selectedImage, setSelectedImage] = useState(getImageUrl(selectedImageDefault))
    ...
    <>
      ...
      <div className="relative h-64 md:h-80 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 text-white">
          <h1 className="text-3xl md:text-4xl font-bold text-balance">{accommodation.typeName}</h1>
          <p className="text-sm md:text-base text-white/90 mt-1">{accommodation.description}</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8 pb-6 border-b border-border">
          <div>
            ...
            <p className="text-3xl font-bold text-teal-600">${parseFloat(accommodation.pricePerNight || '0').toFixed(2)}</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          <Card>
            <CardContent className="p-4 text-center">
              ...
              <p className="font-semibold text-lg">{accommodation.roomSize ? `${accommodation.roomSize} sqm` : 'N/A'}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-xs text-muted-foreground mb-1">Max Guests</p>
              <p className="font-semibold text-lg">{accommodation.maxGuests || 'N/A'}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <p className="text-xs text-muted-foreground mb-1">Bed Configuration</p>
              <p className="font-semibold text-lg">{accommodation.bedConfiguration || 'N/A'}</p>
            </CardContent>
          </Card>
        </div>
        ...
        {/* Description */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold mb-3 text-foreground">About This Accommodation</h2>
          <p className="text-muted-foreground leading-relaxed">{accommodation.description || 'No description available'}</p>
        </div>
