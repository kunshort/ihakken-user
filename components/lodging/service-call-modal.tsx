...
import { AlertCircle, CheckCircle, Clock, Loader2, LucideIcon, Phone, RefreshCw, RotateCw, Shield, ShoppingCart, Users, Utensils, Wrench, XCircle } from "lucide-react"
...
export function CallServiceModal({
    ...
    const handleForceConnected = () => {
        console.log('🔧 Manually forcing connected state');
        updateCallStatus('in-progress');
        toast.info('Manually advanced to connected state');
    }
                ...
                <DialogHeader className="shrink-0 px-6 pt-6">
                    <div className="flex items-center justify-between">
                        <DialogTitle className="flex items-center gap-2 text-xl">
                            ...
                            {activeCall ? 'Active Call' : 'Call a Staff'}
                            {activeCall && (
                    ...
                    {!activeCall ? (
                        <div className="space-y-6">
                                        ...
                                        >
                                            ...
                                            {!isAvailable && (
                                                <div className="flex flex-col items-center gap-1 mt-1">
                                                    <span className="text-xs font-medium text-white px-2 py-0.5 bg-gray-400 rounded-full">
                                                        Unavailable
                                                    </span>
                                                </div>
                                            )}
                            ...
                            {formattedServices.length > 0 && (
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-gradient-to-r from-[#128C7E]/10 to-[#25D366]/10 border border-[#128C7E]/20 rounded-xl p-4">
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm font-medium text-[#128C7E]">Available</span>
                                            <span className="text-xl font-bold text-[#128C7E]">
                                                {formattedServices.filter(s => s.available).length}
                                            </span>
                                        </div>
                                        <p className="text-xs text-[#128C7E] mt-1">Ready for calls</p>
                                    </div>
                        ...
                        <div className="space-y-4">
                            {/* Call Header Card */}
                            <div className="p-6 rounded-2xl bg-gradient-to-r from-[#128C7E]/10 to-[#25D366]/10 border border-[#128C7E]/20">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-3">
                                            <div className="p-2 bg-[#128C7E] rounded-full">
                                                <Phone className="w-5 h-5 text-white" />
                                            </div>
                                            <div>
                                                <h4 className="font-bold text-lg text-gray-800">{activeCall.serviceName}</h4>
                                                <div className="flex items-center gap-2 text-sm text-gray-600">
                                                    <span className="bg-[#128C7E]/10 text-[#128C7E] px-2 py-1 rounded-full text-xs font-medium">
                                                        Audio Call
                                                    </span>
                                                    <span className="text-xs">•</span>
                                                    <span className="text-xs">
                                                        Started {new Date(activeCall.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <div className="text-xs text-gray-500 font-mono">
                                            Room: {activeCall.roomName?.split('_')[1]?.substring(0, 8) || 'Unknown'}
                                        </div>
                                    </div>
                                </div>
                            </div>
